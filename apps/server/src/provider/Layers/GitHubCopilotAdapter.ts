/**
 * GitHubCopilotAdapterLive — GitHub Copilot CLI (`copilot --acp`) via ACP.
 *
...
import {
  GitHubCopilotAskQuestionRequest,
  GitHubCopilotCreatePlanRequest,
  GitHubCopilotUpdateTodosRequest,
  extractAskQuestions,
  extractPlanMarkdown,
  extractTodosAsPlan,
} from "../acp/GitHubCopilotAcpExtension.ts";
import { type GitHubCopilotAdapterShape } from "../Services/GitHubCopilotAdapter.ts";
import { resolveGitHubCopilotAcpBaseModelId } from "./GitHubCopilotProvider.ts";
...
const ACP_IMPLEMENT_MODE_ALIASES = ["code", "copilot", "default", "chat", "implement"];
...
export interface GitHubCopilotAdapterLiveOptions {
   ...
   * Defaults to the legacy built-in instance id (`githubCopilot`).
   */
   ...
   * the `githubCopilotSettings` captured at construction.
   *
   ...
   */
  readonly resolveSettings?: Effect.Effect<GitHubCopilotSettings>;
}
...
export function makeGitHubCopilotAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const logNative = (
      ...
      method: string,
      payload: unknown,
      _source: "acp.jsonrpc" | "acp.githubCopilot.extension",
    ) =>
      Effect.gen(function* () {
    ...
    const emitPlanUpdate = (
      ...
      rawPayload: unknown,
      source: "acp.jsonrpc" | "acp.githubCopilot.extension",
      method: string,
    ) =>
      Effect.gen(function* () {
    ...
    const startSession: GitHubCopilotAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          let ctx!: GitHubCopilotSessionContext;
          ...
          // Resolve the GitHubCopilotSettings used to spawn the ACP child. Production
          ...
          // `updateSettings({ providers: { githubCopilot: { binaryPath } } })` calls
          // actually take effect when the next session spawns.
          const effectiveGitHubCopilotSettings = options?.resolveSettings
            ? yield* options.resolveSettings
            : githubCopilotSettings;

          const acp = yield* makeGitHubCopilotAcpRuntime({
            githubCopilotSettings: effectiveGitHubCopilotSettings,
              ...
              (params) =>
                Effect.gen(function* () {
                  ...
                  yield* offerRuntimeEvent({
                    ...
                    raw: {
                      source: "acp.githubCopilot.extension",
                      method: "githubCopilot/ask_question",
                      payload: params,
                    },
                  });
              ...
              (params) =>
                Effect.gen(function* () {
                  ...
                  yield* offerRuntimeEvent({
                    ...
                    raw: {
                      source: "acp.githubCopilot.extension",
                      method: "githubCopilot/create_plan",
                      payload: params,
                    },
                  });
          ...
          const session: ProviderSession = {
            ...
            runtimeMode: input.runtimeMode,
            cwd,
            model: githubCopilotModelSelection?.model,
            threadId: input.threadId,
            resumeCursor: {
